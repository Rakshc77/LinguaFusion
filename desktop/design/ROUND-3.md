...
motion helpers), then handoff/ROUND-3.md in full. Reference mockups are in
"Transcription Looks.dc.html", Round 3 (#3a–#3h) — match their layout, copy, and
states exactly.

Implement everything in ROUND-3.md:

PART A — two themes, additive only (one block per CSS file + one LF_THEMES row):
  A1 brutalist-web (PC/Web set) — do NOT overwrite existing mobile 'brutalist'.
  A2 kiosk (both sets, kiosk-flagged) — big type, single huge tap target, room-legible.
  Each must restructure LAYOUT via linguafusion-structure.css, not color alone.

PART B — five features (new markup + state + logic, all theme-aware, all offline):
  B1 Bilingual PDF export — client-side/offline PDF, side-by-side EN/target,
     timestamps + speaker turns, repeating header/footer, correct pagination.
  B2 Formality tiers — casual/neutral/formal per translation, one LLM call returns
     all three, selection persists + seeds from a Settings default.
  B3 Auto-detect reveal — animated language lock-on with ISO code + confidence %;
     low-confidence path asks to confirm; sets STT source + translation direction.
  B4 Conversation mode — flat-on-table split, top pane rotated 180°, center mic,
     auto-routes utterances by detected language (reuses B3).
  B5 Ambient ticker — glanceable always-listening widget + in-app panel; current
     line prominent, older lines faded; tap to pause; privacy dot.
  B6 Capture mode — push-to-talk vs continuous as a first-class recorder state
     (segmented control changes button size/label/caption + info card); remember
     last mode; Space-key support on desktop.

RULES:
- Read --lf-* tokens for all color/font/radius/shadow. No hardcoded values
  (exceptions: B2's 3 fixed register hues, B4's 180° rotation).
- No network calls — PDF, detection, translation, and registers all run locally.
- Honor prefers-reduced-motion (B3 reveal, B6 hold pulse, B5 ticker).
- Follow the per-feature Acceptance checklists and the Global acceptance list;
  compare each build against its #3x mockup before marking done.

Build in this order: A1+A2 → B6 → B3 → B4/B2/B1/B5.
```
