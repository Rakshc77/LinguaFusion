...
## Paste-ready prompt for Gemini / Claude Code

```
Read handoff/THEMING.md first (token contract, .lf-* helpers, lf-* layout hooks,
motion helpers), then handoff/ROUND-5.md in full. Reference mockups are in
"Transcription Looks.dc.html": #3a, #4a, #4c, #5p1, #5p4, #5p5, #5m2, #5m3, #5m4,
#5m5 — match their layout, type, and color exactly.

Add these TEN themes to the existing framework. Each is additive: one block in
linguafusion-themes.css + one in linguafusion-structure.css + one in
linguafusion-motion.css + one LF_THEMES row. Do NOT rewrite components. Do NOT
change any existing theme.

PC + Web set (platform:'pc'):
  brutalist-web  (#3a)  — hard concrete, 3px rules, orange; 3-col controls, 2-col result.
                          NOTE: already specified in ROUND-3.md §A1 — if built, skip.
  reading-room   (#4a)  — Newsreader literary serif, ivory; centered masthead +
                          single reading column with marginal timestamps.
  gallery        (#4c)  — Syne display, near-black + lime; left rail + big type column.
  aurora-glass   (#5p1) — Space Grotesk, glass panels over aurora, teal; glass recorder pill.
  editorial-luxe (#5p4) — Bodoni + Spectral, ivory + gold; centered masthead + 2-col split.
  blueprint      (#5p5) — JetBrains Mono + Space Grotesk, navy drafting grid, cyan;
                          label-tab bordered boxes + coordinate row.

Mobile set (platform:'mobile'):
  neon-arcade    (#5m2) — Unbounded, synthwave; neon cyan/magenta outlined cards, glow mic.
  warm-minimal   (#5m3) — Cormorant, cream + terracotta; single centered column, outline mic.
  bold-mono      (#5m4) — Space Mono, black/white; full-bleed EN/DE split, DE inverted, 2px rules.
  nature-calm    (#5m5) — Lora, sage; soft rounded stack, filled sage translation card.

RULES:
- Read --lf-* tokens for ALL color/font/radius/shadow. No hardcoded values.
- CRITICAL: each theme must RESTRUCTURE layout via linguafusion-structure.css,
  not just recolor — the structural signature per theme is in ROUND-5.md. Verify
  each against its mockup.
- Add the fonts to the themes.css @import (self-host for the offline build):
  Newsreader, Syne, Space Grotesk, JetBrains Mono, Bodoni Moda, Spectral,
  Unbounded, Cormorant Garamond, Space Mono, Lora, Archivo, Big Shoulders Display.
- Set color-scheme from the dark flag (gallery, aurora-glass, blueprint, neon-arcade).
- Honor prefers-reduced-motion.

ACCEPTANCE: switch through all themes; confirm each new one changes LAYOUT + type
+ color (not color alone), text meets WCAG AA on its surface, nothing clips at
360–430px and ≥1024px. Compare each build to its #-mockup before marking done.
Keep it token-driven: a later theme = one block per CSS file + one LF_THEMES row.
```
