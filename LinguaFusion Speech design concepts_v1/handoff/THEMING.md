...
Claude Code (or Gemini) can wire this in from the package directly. Drop this
folder into your repo and paste a prompt like:

```
Read handoff/THEMING.md and every handoff/linguafusion-* file.
Integrate this theme + motion framework into the LinguaFusion app.

CRITICAL: themes are NOT just color swaps. linguafusion-structure.css changes
LAYOUT per theme, but only if the markup wears the hook classes. So:

1. Load the three stylesheets in THIS order: themes.css, structure.css,
   motion.css. Call initTheme() before first paint (no theme flash).
2. Add the layout hooks to Speech and to Translate/Reader/OCR/Notes:
   lf-speech (root), lf-signature (empty slot), lf-speech-head,
   lf-speech-controls, lf-recorder, lf-result. Verify the structural themes
   actually restructure: swiss = grids, terminal = prompt line, editorial-split
   = dark rail, zen = single centered column, broadsheet = columns+dateline,
   sunset = white sheet on gradient, brutalist = hard borders/shadows.
3. Make every component read --lf-* tokens (adopt .lf-* helpers or map onto
   existing components). Remove ALL hardcoded colors/fonts/radii/shadows.
4. Replace Settings > Appearance A/C toggle with the grouped picker:
   mobile build -> picker(el,{platform:'mobile'}); desktop+web -> {platform:'pc'}.
5. Wire motion: enterScreen on route change, importIn on import, exportOut on
   export/save, revealResult on new transcript/translation lines, setRecording
   + setWaveLive on record start/stop.
6. Self-host the fonts in THEMING.md; remove the @import (offline app).

ACCEPTANCE: switch through all 16 themes and confirm each changes LAYOUT +
type + color (not color alone), text meets WCAG AA contrast on its surface, and
no element is clipped. Compare against the option mockups. Keep it token-driven
so a 17th theme = one block in each CSS file + one LF_THEMES row.
```

Point it at `THEMING.md` first — that's the entry point.
